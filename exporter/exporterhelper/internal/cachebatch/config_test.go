...
func TestConfig_Validate(t *testing.T) {
	...
	cfg = newTestConfig()
	cfg.CacheSize = 0
	require.EqualError(t, cfg.Validate(), "`cache_size` must be positive")
	...
	require.EqualError(t, cfg.Validate(), "`wait_for_result` is not supported with a persistent cache configured with `storage`")

	cfg = newTestConfig()
	cfg.Sizer = request.SizerTypeBytes
	cfg.StorageID = &storageID
	require.EqualError(t, cfg.Validate(), "persistent cache configured with `storage` only supports `requests` sizer")
	...
	cfg = newTestConfig()
	cfg.CacheSize = cfg.Batch.MinSize - 1
	require.EqualError(t, cfg.Validate(), "`min_size` must be less than or equal to `cache_size`")
