...
func (qb *partitionBatcher) Consume(ctx context.Context, req request.Request, done cache.Done) {
...
// Start starts the goroutine that reads from the cache and flushes asynchronously.
...
// Shutdown ensures that cache and all Batcher are stopped.
...
func (qb *partitionBatcher) flush(ctx context.Context, req request.Request, done cache.Done) {
	qb.stopWG.Add(1)
	qb.wp.execute(func() {
		defer qb.stopWG.Done()
...
func newRefCountDone(done cache.Done, refCount int64) cache.Done {
	return &refCountDone{
		done:     done,
		refCount: refCount,
	}
