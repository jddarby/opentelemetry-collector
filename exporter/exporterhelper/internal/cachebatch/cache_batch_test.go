...
func newFakeEncoding(mr request.Request) cache.Encoding[request.Request] {
	return &fakeEncoding{mr: mr}
}

func TestCacheBatchStopWhileWaiting(t *testing.T) {
	sink := requesttest.NewSink()
	cfg := newTestConfig()
	cfg.NumConsumers = 1
	cfg.Batch = nil
	qb, err := NewCacheBatch(newFakeRequestSettings(), cfg, sink.Export)
...
func TestCacheBatchHappyPath(t *testing.T) {
	cfg := newTestConfig()
...
func TestCacheBatchHappyPathLegacyBatcher(t *testing.T) {
	// Set up the config so that the request is accepted in the cache
	// because the bytes size is used for the cache,
	...
	sink := requesttest.NewSink()
	qb, err := NewCacheBatchLegacyBatcher(newFakeRequestSettings(), cfg, sink.Export)
	require.NoError(t, err)
	require.NoError(t, qb.Start(context.Background(), componenttest.NewNopHost()))
...
func TestCacheBatchPersistenceEnabled(t *testing.T) {
	cfg := newTestConfig()
	storageID := component.MustNewIDWithName("file_storage", "storage")
	cfg.StorageID = &storageID
	qb, err := NewCacheBatch(newFakeRequestSettings(), cfg, sendertest.NewNopSenderFunc[request.Request]())
	require.NoError(t, err)
...
func TestCacheBatchPersistenceEnabledStorageError(t *testing.T) {
...
func TestCacheBatchPersistentEnabled_NoDataLossOnShutdown(t *testing.T) {
	cfg := newTestConfig()
	cfg.NumConsumers = 1
	storageID := component.MustNewIDWithName("file_storage", "storage")
	cfg.StorageID = &storageID
	...
	qb, err := NewCacheBatch(qSet, cfg, func(context.Context, request.Request) error {
		<-done
	...
	// Invoke cachedRetrySender so the producer will put the item for consumer to poll
	require.NoError(t, qb.Send(context.Background(), mockReq))

	// first wait for the item to be consumed from the cache
	assert.Eventually(t, func() bool {
		return qb.cache.Size() == 0
	}, 1*time.Second, 10*time.Millisecond)

	// shuts down the exporter, unsent data should be preserved as in-flight data in the persistent cache.
	close(done)
	require.NoError(t, qb.Shutdown(context.Background()))
	...
	qb, err = NewCacheBatch(qSet, cfg, sink.Export)
	require.NoError(t, err)
	require.NoError(t, qb.Start(context.Background(), host))
...
func TestCacheBatchNoStartShutdown(t *testing.T) {
	qs, err := NewCacheBatch(newFakeRequestSettings(), newTestConfig(), sendertest.NewNopSenderFunc[request.Request]())
	require.NoError(t, err)
	assert.NoError(t, qs.Shutdown(context.Background()))
}

func TestCacheBatch_Merge(t *testing.T) {
...
func TestCacheBatch_BatchExportError(t *testing.T) {
	tests := []struct {
		name             string
		batchCfg         *BatchConfig
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// the batch should be dropped since the cache doesn't have re-queuing enabled.
			assert.Eventually(t, func() bool {
...
func TestCacheBatch_MergeOrSplit(t *testing.T) {
	sink := requesttest.NewSink()
	cfg := newTestConfig()
...
func TestCacheBatch_MergeOrSplit_Multibatch(t *testing.T) {
	sink := requesttest.NewSink()
	cfg := newTestConfig()
	cfg.Batch = &BatchConfig{
		FlushTimeout: 100 * time.Millisecond,
	...
	qb, err := NewCacheBatch(set, cfg, sink.Export)
	require.NoError(t, err)
	require.NoError(t, qb.Start(context.Background(), componenttest.NewNopHost()))
...
func TestCacheBatch_Shutdown(t *testing.T) {
	sink := requesttest.NewSink()
	qb, err := NewCacheBatch(newFakeRequestSettings(), newTestConfig(), sink.Export)
	require.NoError(t, err)
	require.NoError(t, qb.Start(context.Background(), componenttest.NewNopHost()))
...
func TestCacheBatch_BatchBlocking(t *testing.T) {
	sink := requesttest.NewSink()
	cfg := newTestConfig()
...
func TestCacheBatch_DrainActiveRequests(t *testing.T) {
	sink := requesttest.NewSink()
	cfg := newTestConfig()
	cfg.WaitForResult = true
	cfg.Batch.MinSize = 2
...
func TestCacheBatchTimerResetNoConflict(t *testing.T) {
	sink := requesttest.NewSink()
	cfg := newTestConfig()
	cfg.WaitForResult = true
	cfg.Batch.MinSize = 8
...
func TestCacheBatchTimerFlush(t *testing.T) {
	if runtime.GOOS == "windows" {
