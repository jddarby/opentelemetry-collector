...
func createAndStartTestPersistentQueue(t *testing.T, sizer request.Sizer[uint64], capacity int64, numConsumers int,
	...
	pq := newPersistentCache[uint64](persistentCacheSettings[uint64]{
		mode:      ModeQueue,
		sizer:     sizer,
		capacity:  capacity,
	...
	ac := newAsyncCache(pq, numConsumers, func(ctx context.Context, item uint64, done Done) {
...
func createTestPersistentQueueWithClient(client storage.Client) *persistentCache[uint64] {
	pq := newPersistentCache[uint64](persistentCacheSettings[uint64]{
		mode:      ModeQueue,
		...
		id:        component.NewID(exportertest.NopType),
		telemetry: componenttest.NewNopTelemetrySettings(),
	}).(*persistentCache[uint64])
	pq.initClient(context.Background(), client)
	return pq
}

func createTestPersistentQueueWithRequestsCapacity(tb testing.TB, ext storage.Extension, capacity int64) *persistentCache[uint64] {
	return createTestPersistentQueueWithCapacityLimiter(tb, ext, request.RequestsSizer[uint64]{}, capacity)
}

func createTestPersistentQueueWithItemsCapacity(tb testing.TB, ext storage.Extension, capacity int64) *persistentCache[uint64] {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < 10; i++ {
				...
				if i < 5 {
					...
					require.ErrorIs(t, result, ErrCacheIsFull)
				}
			}
...
func TestPersistentBlockingQueue(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			consumed := &atomic.Int64{}
			ac := newAsyncCache(pq, 10, func(_ context.Context, _ uint64, done Done) {
				consumed.Add(1)
				done.OnDone(nil)
			})
			host := hosttest.NewHost(map[component.ID]component.Component{
...
func TestPersistentQueue_CorruptedData(t *testing.T) {
	cases := []struct {
		...
		corruptItemsToDispatchKey          bool
		...
		},
		{
			...
			corruptItemsToDispatchKey: true,
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if c.corruptItemsToDispatchKey {
				require.NoError(t, ps.client.Set(context.Background(), itemsToDispatchKey, badBytes))
			}
...
func TestPersistentQueue_ItemsCapacityUsageRestoredOnShutdown(t *testing.T) {
	...
	require.ErrorIs(t, pq.Offer(context.Background(), uint64(25)), ErrCacheIsFull)
	assert.Equal(t, int64(100), pq.Size())
...
func requireCurrentlyDispatchedItemsEqual(t *testing.T, pq *persistentCache[uint64], compare []uint64) {
	pq.mu.Lock()
	defer pq.mu.Unlock()
