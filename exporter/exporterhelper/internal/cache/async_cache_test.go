...
func TestAsyncMemoryCache(t *testing.T) {
	...
	ac := newAsyncCache(newMemoryQueue[int64](memoryQueueSettings[int64]{sizer: sizerInt64{}, capacity: 100}),
...
func TestAsyncMemoryCacheBlocking(t *testing.T) {
	consumed := &atomic.Int64{}
	ac := newAsyncCache(
...
func TestAsyncMemoryWaitForResultCacheBlocking(t *testing.T) {
	consumed := &atomic.Int64{}
	ac := newAsyncCache(
...
func TestAsyncMemoryCacheBlockingCancelled(t *testing.T) {
...
func BenchmarkAsyncMemoryCache(b *testing.B) {
	...
	ac := newAsyncCache(q, 1, func(_ context.Context, _ int64, done Done) {
		consumed.Add(1)
		done.OnDone(nil)
	})
