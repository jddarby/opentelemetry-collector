...
func TestMemoryQueue(t *testing.T) {
	...
	// should not be able to send to the full cache
	require.ErrorIs(t, q.Offer(context.Background(), 4), ErrCacheIsFull)
	assert.EqualValues(t, 4, q.Size())
...
func TestMemoryQueueZeroCapacity(t *testing.T) {
	q := newMemoryQueue[int64](memoryQueueSettings[int64]{sizer: sizerInt64{}, capacity: 1})
	require.NoError(t, q.Offer(context.Background(), 1))
	require.ErrorIs(t, q.Offer(context.Background(), 1), ErrCacheIsFull)
	require.NoError(t, q.Shutdown(context.Background()))
}
...
func consume[T any](q readableCache[T], consumeFunc func(context.Context, T) error) bool {
